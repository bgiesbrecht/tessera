...
Oracle adapter live-verified on Oracle 23ai Free, plus three emission bugs the live run caught, now fixed and guarded.
...
- Exemption facts recorded: SYS and `EXEMPT ACCESS POLICY` bypass VPD; SYS and `EXEMPT REDACTION POLICY` (held via DBA, so SYSTEM) bypass redaction, so the redaction check uses a non-DBA reader. `adapters/tests/live_oracle.py` documents the run.

## [0.14.0] — 2026-08-17

Fourth adapter: **Oracle** (ADR-033). A third native platform, proving IR portability against a different mechanism set (Virtual Private Database, Data Redaction, GRANT, none resembling the UC/Snowflake primitives).
...
- **Portability finding (ADR-033):** Oracle FULL redaction cannot carry an arbitrary replacement, so `Redact` lowers to REGEXP, a mask-value-fidelity fact for future Mask/Hash work. The `DBMS_REDACT` `expression` is a PL/SQL string literal, so inner quotes are doubled (a real bug caught and guarded in test).
...
- **`ABAC_ROW_ACCESS_TABLE_TAG` diagnostic**: row-access emission now notes that the driving tag must be set on the **table** (masking tags the column), and names the discriminator column that must exist; both facts were discovered in live verification.
...
- Stage 4 ABAC vocabulary in `spec/v0/ontology.ttl`, `context.jsonld`, `schema.json`, implementing ADRs 018–021. `tessera:AttributeAxis` class; four well-known axes (`sensitivityAxis` hierarchical; `dataSubjectAxis`, `regulatoryRegimeAxis`, `businessDomainAxis` flat); `byScope` selector with `scope` / `except` / `matching` properties; `AttributeMatcher` class.
