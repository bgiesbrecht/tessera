...
Verified 2026-05-19 against `acme.tpch.orders_abac` on `adb-984752964297111.azuredatabricks.net` (Azure). Brice was **not** in `acme_all_priority_ops` during all runs, so we ran Scenario 2 (the load-bearing one) directly without needing Scenario 1 first.
...
1. Pre-checked Brice's group membership via `is_account_group_member('acme_all_priority_ops')`, which returned `false` (Scenario 2 state).
...
The structural match for Mechanism A is tight. The Tessera derivation reached the same shape as Brice's primary recommendation independently, validating the blind-derivation property of the worked-example framework.
...
- (a) is not expressible in v0 because the transformation vocabulary is closed (`Redact`, `Mask`, `Hash`, `Tokenize`, `Bucketize`), none of which carry conditional logic.
...
So this `GRANT SELECT` is **table-creation deployment scaffolding**, not a policy concern. Tessera's IR correctly does not model it. It is no more a policy concern than the `CREATE TABLE` or `CREATE GOVERNED TAG` statements that also appear in the script.
...
Both are legitimate readings of the inputs §3 framing. The Phase 1 inputs §0.3 and §4.1 specified catalog scope ("`scope: catalog:acme`"), and the Tessera derivation followed the spec. Brice's existing impl chose schema scope, perhaps reflecting a tighter intent than the inputs captured.
...
This is the **third occurrence** of grant-emission asymmetry between Tessera derivation and existing implementations across the worked-example series. The previous pattern from issue #10 `policy-execute-grants`: existing notebook implementations emitted GRANT EXECUTE; Tessera derivations omitted it. Here it is inverted: existing impl omits, Tessera emits.
...
The SQL emission is structurally equivalent; the IR and supporting documents are what's new.
...
- **The alternate Mechanism B example** (`orders_clerk_mask_by_group`). Tessera v0 cannot express this naturally (see §3.2); track it as a v1 candidate if Mechanism B becomes a real need.
...
The deployment observation and the structural comparison surfaced two v1 candidates:
...
The blind-derivation property held through Phase 2; the structural match with Brice's primary Mechanism A is tight; the Mechanism B alternative surfaced a design dimension the Phase 1 inputs did not anticipate. The framework's "exercises drive design" principle held again here.
