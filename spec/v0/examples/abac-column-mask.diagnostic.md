...
The exercise's output is **not** yet observable, since it requires deploying both policies and querying as a non-privileged principal to surface the cross-policy combination behavior (ADR-019's deferred α/β/γ decision). That observation lives in the Phase 3 comparison document. Phase 2 establishes that the design *can be expressed*; Phase 3 establishes what *happens* when both are deployed.

Three structural findings from the Phase 2 derivation itself, independent of any Phase 3 observation:
...
## 5. The observation Phase 3 produces

The Phase 3 comparison document is currently a stub. After deployment, it captures:

- What Databricks does for a principal **in** `acme_all_priority_ops` (predicted: pass-through, both policies have the same EXCEPT, so both bypass, and the principal sees the real clerk value).
- What Databricks does for a principal **not** in `acme_all_priority_ops`. Both Policy A and Policy B apply; both transformations are defined; what value does Databricks return?
...
A secondary observation to measure: **policy attachment ordering**. Does it matter whether Policy A is created before Policy B, or vice versa? If yes, ADR-019 needs to address ordering semantics. Phase 3 should deploy in both orders to test.
