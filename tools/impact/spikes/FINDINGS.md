...
- **Opaque operators.** Conditions beyond `in`/`eq` on a single column (notably `exists-in-dataset` / ACL selectors) are not enumerable; the spike surfaces them as an explicit note rather than mis-evaluating — the ADR-001 data line reappearing exactly where expected.
...
**Conclusion (Layer 2, within-policy).** This is the promising direction. It directly answers "did this edit open or close access, and for which requests" for the attribute/enumerable slice, needs no solver and no combining commitment, stays behind ADR-001, and is strictly more informative than the shipped C6. It generalizes C6 from a per-rule heuristic to a per-request decision diff, and subsumes part of C3 (an unreachable rule is one that changes no request's decision).

---

## Recommendation

1. **Promote Spike 2 toward a real check** — within-policy request-space diff with witnesses. It's the highest-value, lowest-risk outcome and it's what the original concern ("innocent change quietly opens/closes access") actually needs. Next step would be hardening (larger spaces, range conditions via interval classes) and deciding whether it replaces or complements C6.
2. **Cross-policy (the bracket / `[floor, ceiling]` model from the design chat) is the following step** — same enumeration machinery, evaluating the corpus under the floor/ceiling bounds so guarantees are implementation-independent and the ambiguous zone is flagged. That step wants the ADR that states the faithfulness assumptions (not a combining algorithm).
3. **Hold Spike 1 (graph)** until/unless the corpus adopts CURIE attribute values. The subsumption win is real but contingent; today the graph would duplicate Python and risk the reflexive-match trap.
