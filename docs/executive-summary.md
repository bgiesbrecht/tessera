...
The shared meaning lives in the portable representation. The platform-specific mechanics live in the adapters. The result is **semantic interoperability of policy**: an agreement that "PII," "fraud investigation purpose," "EU residency," and "audit-log obligation" mean the same thing wherever they are enforced.

## What this is, and is not, with respect to Unity Catalog

Unity Catalog is the source of truth for governance inside a Databricks environment. This project does not change that. For customers running Databricks alone, this project is not relevant, because Unity Catalog already does what they need.
...
The project is a customer-enablement initiative, not a standards effort. We are not proposing that the industry adopt a new governance standard. We are proposing to give customers who run multi-platform environments a tool that solves a real and continuous problem they face today.
...
- **Runtime interoperability.** A query-time gateway imposing a single authorization decision across platforms. A different product category with different trade-offs; the project does not pursue it.

These boundaries are explicit on purpose. The project scopes to the problem it can solve well.
...
- Cross-platform consistency demonstrated by diff, not by expert assertion, and usable as the basis for audit, migration, and steady-state operation.
