...
If the answer is yes, the spec earns confidence as a real abstraction over the problem. If the answer is no (Tessera can't express something the direct implementation captures, or the derived implementation behaves subtly differently), that finding matters more than the artifact, because it tells us where the spec needs revision before more is built on top of it.
...
The phasing holds: the exercise's value depends on Phase 2 happening without sight of the answer.
...
- Any relevant classifications already applied to the resource (whether it's tagged PII, Financial, etc.).
...
The edge cases matter as much as the headline intent. Brice should describe, without showing the code, what edge cases the existing implementation handles. Examples of the kinds of questions to ask:
...
- **Capture obligations explicitly.** For audit-log, notify, and watermark, use the obligation classes from the ontology (`tessera:AuditLog`, `tessera:Notify`, `tessera:Watermark`).
...
Edge cases discovered during Phase 1 should each correspond to an explicit element in the policy file. If an edge case cannot be expressed within v0 of the spec, surface it explicitly (see §2.4 below).
...
Produce the SQL that a Tessera-derived Snowflake adapter would emit for this policy. This output is what will be compared against the existing implementation.
...
- The condition algebra includes `purposeIn`. Snowflake supports session tags via `ALTER SESSION SET TAG`, which can be read in policy bodies; that is how purpose binding gets enforced.
...
Per §5.3 of the technical design, every emission produces a diagnostic report. Even for a hand-derived example, produce this report explicitly. It forces honest accounting of what the framework can and cannot do.
...
If both implementations agree on every test case, the Tessera derivation is *behaviorally equivalent* to the direct implementation. That is the criterion the project actually cares about.
