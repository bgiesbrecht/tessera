...
**Grounding:** ADR-023. **Confidence:** `PROVEN` for the scope/attribute overlap; the *behavioral* resolution (which policy "wins") is reported as the ADR-023 rule, not computed against data.
...
- **Stage 4 — C4.** Cross-policy overlap. Requires the pairwise scope/attribute machinery; validates against Exercise 2 and grounds against ADR-023.
...
**Implementation status (2026-08-04).** Stages 1–3 are built in `tools/impact/` — kernel (`kernel.py`, ontology-grounded via rdflib), diff checks C1/C2/C3/C5/C6 and the standing L1 dead-rule lint (`checks.py`), findings/report model, and a CLI (`__main__.py`) with a `--lint` single-corpus mode. Corpus discovery is git-tracked by default with a `--corpus DIR` filesystem override and an explicit `--baseline`/`--proposed` file mode (§9.1). Report output is deterministic (findings emitted in source/rule order). The §6 exercises run as acceptance tests, alongside kernel, C3-reachability, lint, CLI, and a demo-reproducibility test (`tools/impact/tests/`). The dead-rule timeline demo is generated by `tools/impact/demo/build_timeline_demo.py` and its output is regression-tested for drift. Run with `./.venv/bin/python -m pytest tools/ -q`. Stage 4 (C4 cross-policy overlap) remains.
