...
- At the end, the "Demo or production?" question matters for scope; answer it deliberately, not by default.
...
Does the policy depend on role or group inheritance? If so, describe it (for example, "members of group A also have the privileges of group B") without referencing how that inheritance is implemented.
