...
A Databricks tag with key `abac_column` and value `clerk` is applied to the column. The mechanism (governed tag vs. ordinary column tag) is Brice's choice in §9 setup, and both work with `has_tag_value`. The exercise prefers governed tags for the audit/integrity benefits, but ordinary tags suffice for the demo.
...
The matching predicate is identical, so the conflict is **pure-overlap, different effect**: the simplest shape for surfacing cross-policy combination. What Databricks ABAC does when both policies are attached to the same column is what Stage 3 observes. The observation discriminates among ADR-019's three resolution paths:
