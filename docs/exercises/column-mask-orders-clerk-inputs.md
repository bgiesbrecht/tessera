...
None on the column. The mask is gated by principal group membership, not by data classification: an *attribute-on-principal* pattern, not an *attribute-on-data* pattern.
