...
The codename indirection is preserved; it is the feature that makes this pattern interesting to Tessera (data-driven indirection between principal and resource).
...
**5.9 Secondary-roles activation.** Not relevant: `CURRENT_USER()` is unaffected, which is why this pattern is preferable to `IS_ROLE_IN_SESSION`-based policies on Snowflake. Phase 3 should confirm by toggling `USE SECONDARY ROLES ALL | NONE` and verifying row counts are unchanged.
