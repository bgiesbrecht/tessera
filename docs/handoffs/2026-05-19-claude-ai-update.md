...
3. **Pre-ABAC column-mask exercise ran** as a single-pass / combined-input exercise (Brice shared the existing SQL up front). It surfaced **ADR-022**, the over-tight transformation constraint in the schema, which now correctly says `transformation` is required iff `effect: transform`. A small spec correction.
...
`DECISIONS.md` has the full text. ADR-023 and ADR-022 both corrected prior decisions, and the discipline of "ADRs supersede, not edit" held cleanly.
...
3. **Mechanism-A-vs-B documentation.** ADR-022 fixed the schema; the technical design §4.2.2 was also updated. But the **design distinction** itself (when do you use Mechanism A vs B, what does Tessera prefer) doesn't yet have a dedicated paragraph. A short addition to the technical design adapter-contract section would let the principle survive turnover.
...
3. **`policy-execute-grants` (issue #10) recurring across all exercises.** Five out of five exercises have surfaced GRANT EXECUTE asymmetry, a candidate to design at v0-time. The design (an `executeGrants` field on Policy that names principals authorized to invoke the policy's compiled UDF) is small.
...
The relationship is complementary; neither side would have produced the current state alone. This update doc is the synchronization point: claude.ai catches up on what's in the repo before contributing the next design layer.
...
2. **SHACL shapes shipped (`spec/v0/shapes.ttl`).** Priority 4 done. Closes the validation pipeline at the semantic layer; covers what JSON Schema cannot (closed vocabularies, IRI/class typing, node-shape composition). The conditional-dependency constraints (baselineGroup↔defaultStrategy, transformation↔effect, etc.) are deliberately deferred to JSON Schema. That split is documented in shapes.ttl and CLAUDE.md.
...
2. **Snowflake's `DEFAULT_SECONDARY_ROLES = ("ALL")` default is a real operator concern, not just a testing-mode oddity.**
   ...
   - Snowflake's own canonical answer for role-discriminating policies is **the mapping-table pattern**: author against a centralized authorization table joined by `CURRENT_USER()` / `CURRENT_ROLE()`, not against role membership directly. That is structurally identical to Tessera's `byDataset` + `PrincipalSetFromTable` (the custom-ACL pattern that drove ADR-003). The Snowflake adapter naturally aligns with Snowflake's recommended pattern when IR authors use `byDataset`.
...
5. **Should `byDataset` be the recommended Snowflake authoring pattern?** IR authors targeting Snowflake could prefer `byDataset` + an ACL table over inline `byIdentity` with per-role bindings, since (a) it sidesteps the secondary-roles issue, (b) it matches Snowflake's documented best practice, and (c) it isolates role-taxonomy changes to a table update instead of policy re-emission. The user-documentation pass needs to decide whether to present `byDataset` as the recommended Snowflake pattern explicitly, or leave it as one of several authoring styles.
...
- **The `byDataset`-as-Snowflake-preferred-pattern framing.** Design input before it lands as user-documentation framing would help. The structural alignment with Snowflake's own best practice reads cleanly in retrospect; the question is whether to lead with it or treat it as one option among several.
...
The morning snapshot anticipated SHACL, Stage 4, and one adapter as multi-day work. All three landed plus a cross-platform live exercise in a single working session. With the contract shape settled (ADR-024) and the worked-example series having pre-validated the IR design, downstream infrastructure work moved much faster than the upstream design work did. The converter and the `custom-acl` adapter involve genuinely new design surfaces, so they may not go as quickly. But the spec → validators → adapters chain that took weeks to design ran through implementation in hours once the design was settled.
