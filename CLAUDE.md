...
- **Thirty-one GitHub issues** total. Resolved/closed: #1, #2, #6, #10 (ADR-025), #15 (ADR-026). Open from worked-example findings: #3 (deferred-not-needed-yet), #4, #5, #7, #8, #9, #11, #12, #13, #14. Open from the 2026-05-19 governance-gap survey: #16–#25 (see `docs/handoffs/2026-05-19-governance-gaps-handoff.md`). Open from the 2026-05-20 migration-cycle work (0.4.0): #26 (adapter reconcile stub), #27 (UC adapter discover/extract), #28 (UDF parameter-collision convention to document), #29 (extracted-IR identifier provenance), #30 (UC ABAC byScope column-mask), #31 (Snowflake ABAC byScope). Phase 2 scoping documents for #19/#21/#25 are still queued.
